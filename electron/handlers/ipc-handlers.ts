...
import { releaseAgentTracking, createAgent } from '../core/agent-manager';
...
function registerAgentHandlers(deps: IpcHandlerDependencies): void {
  ...
  // The PTY/worktree/AgentStatus construction now lives in the reusable
  // `createAgent` in core/agent-manager.ts so season-spawned cast members and
  // UI-spawned agents share one casting code path (no PTY-logic duplication).
  ipcMain.handle('agent:create', async (_event, config: {
    ...
    const status = await createAgent(config, getAppSettings, handleStatusChangeNotification);
    return { ...status, ptyId: status.ptyId };
  });
